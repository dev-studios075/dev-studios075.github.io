...
const visualPanelClass =
  "bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/[0.07] shadow-sm";

// 1. Cost Visualizer
const CostVisualizer = () => (
  <div className={`relative h-24 w-full rounded-xl overflow-hidden p-2.5 font-mono text-[9px] flex flex-col justify-between ${visualPanelClass}`}>
    <div className="flex justify-between items-center">
      <span className="text-slate-500">OVERHEAD LEDGER</span>
      <span className="font-bold text-emerald-500 dark:text-emerald-400">SAVED</span>
    </div>
    <div className="h-10 w-full relative flex items-end">
      <svg viewBox="0 0 200 40" className="w-full h-full" style={{ fill: "rgba(239,68,68,0.08)" }}>
        ...
        <line x1="0" y1="10" x2="200" y2="10" className="stroke-slate-200 dark:stroke-white/5" strokeWidth="0.5" />
        <line x1="0" y1="25" x2="200" y2="25" className="stroke-slate-200 dark:stroke-white/5" strokeWidth="0.5" />
      </svg>
    </div>
...
const ErrorsVisualizer = () => {
  ...
  return (
    <div className={`h-24 w-full rounded-xl p-2.5 flex flex-col justify-between font-mono text-[9px] ${visualPanelClass}`}>
      <div className="font-semibold uppercase tracking-wider text-slate-500">ERROR SANITIZER</div>
      <div className="flex-1 flex items-center justify-center gap-2">
        <div className={`w-6 h-6 rounded-full border flex items-center justify-center transition-all duration-300 ${
          pulse ? "border-emerald-500" : "border-slate-300 dark:border-slate-700"
        }`} style={pulse ? { background: "rgba(52,211,153,0.15)", color: "#34d399" } : { color: "transparent" }}>
          ✓
        </div>
        <div className="text-left">
          <div className="font-bold text-[9px] text-slate-800 dark:text-slate-200">0 ANOMALIES</div>
          <div className="text-[7.5px] text-slate-500">100% inputs sanitized</div>
        </div>
      </div>
      <div className="text-[7px] text-right text-slate-500 dark:text-slate-600">No manual entry exceptions</div>
    </div>
  );
};
...
const EfficiencyVisualizer = () => (
  <div className={`h-24 w-full rounded-xl p-2.5 flex flex-col justify-between font-mono text-[9px] ${visualPanelClass}`}>
    <div className="font-semibold uppercase tracking-wider text-slate-500">UTILIZATION RATE</div>
    <div className="flex items-end justify-around h-12 pt-2 gap-4">
      <div className="flex flex-col items-center flex-1">
        <div className="text-[7.5px] font-bold mb-1 text-slate-500 dark:text-slate-400">67%</div>
        <div className="w-full rounded-t h-6 bg-slate-200 dark:bg-slate-800" />
        <div className="text-[6.5px] mt-1 uppercase text-slate-500 dark:text-slate-600">Before</div>
      </div>
      <div className="flex flex-col items-center flex-1">
...
const LatencyVisualizer = () => {
  ...
  return (
    <div className={`h-24 w-full rounded-xl p-2.5 flex flex-col justify-between font-mono text-[9px] relative overflow-hidden ${visualPanelClass}`}>
      <div className="font-semibold uppercase tracking-wider text-slate-500">LATENCY MONITOR</div>
      <div className="flex-1 flex items-center justify-center gap-3">
        ...
        <div className="text-left">
          <div className="font-bold text-[10px] tracking-wider text-slate-900 dark:text-slate-100">{val}</div>
          <div className="text-[7px] text-slate-500">Auto-dispatch delay</div>
        </div>
      </div>
      <div className="text-[7px] text-slate-500 dark:text-slate-600">Threshold: &lt;2.0s</div>
    </div>
  );
};
...
const ScaleVisualizer = () => (
  <div className={`relative h-24 w-full rounded-xl overflow-hidden p-2.5 font-mono text-[8px] ${visualPanelClass}`}>
    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 90">
      <line x1="100" y1="45" x2="30"  y2="20" className="stroke-slate-200 dark:stroke-white/10" strokeWidth="0.8" />
      <line x1="100" y1="45" x2="170" y2="20" className="stroke-slate-200 dark:stroke-white/10" strokeWidth="0.8" />
      <line x1="100" y1="45" x2="40"  y2="70" className="stroke-slate-200 dark:stroke-white/10" strokeWidth="0.8" />
      <line x1="100" y1="45" x2="160" y2="70" className="stroke-slate-200 dark:stroke-white/10" strokeWidth="0.8" />
      ...
      <circle cx="30"  cy="20" r="3.5" className="fill-slate-400 dark:fill-slate-700" />
      <circle cx="170" cy="20" r="3.5" className="fill-slate-400 dark:fill-slate-700" />
      <circle cx="40"  cy="70" r="3.5" className="fill-slate-400 dark:fill-slate-700" />
      <circle cx="160" cy="70" r="3.5" className="fill-slate-400 dark:fill-slate-700" />
    </svg>
    <div className="absolute top-2 left-2 font-semibold uppercase tracking-wider text-[7.5px] text-slate-500">
      OPERATOR CAPACITY
    </div>
...
const UptimeVisualizer = () => (
  <div className={`h-24 w-full rounded-xl p-2.5 flex flex-col justify-between font-mono text-[9px] relative overflow-hidden ${visualPanelClass}`}>
    <div className="font-semibold uppercase tracking-wider text-slate-500">DAEMON LIFELINE</div>
    <div className="flex-1 flex items-center justify-center gap-2">
      ...
      <div className="text-left">
        <div className="font-bold text-[10px] text-slate-900 dark:text-slate-100">99.99% UPTIME</div>
        <div className="text-[7px] text-slate-500">Autonomous loop</div>
      </div>
    </div>
    <div className="text-[7px] text-right text-slate-500 dark:text-slate-600">24/7/365 telemetry</div>
  </div>
);
